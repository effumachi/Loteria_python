# Developed by Fumachi, E. F.
# effumachi_at_gmail.com
# Sistema de sorteio aleatorio para loteria LotoFácil
from random import *
import gc
gc.collect()
...
#----------- Verificação de o jogo sugerido já foi jogado------------------
feitos = open('./DATA/meu_feitos.txt','r')
lfeitos = feitos.readlines()
feitos.close()

feito = []
with open('./DATA/meu_feitos.txt','r') as d:
    for line in d:
        feito.append([int(x) for x in line.split()])
    ...
    #    print('inédito')
    #    print(feito[i])
#----------- Fim da Verificação de o jogo sugerido já foi jogado-----------
jogo2 = []
for i in range(len(lfeitos)):
    #print(feito[i])
    #print(jogo1,'\n')
    if feito[i] == jogo2:
        print('O jogo 1 já saiu na linha',i)
        while len(jogo2) != 15:
            r6 = randint(1,25)
            if conta_6[r6-1]<5:
                if r6 not in jogo2:
